...
import {
  ENEMY_DEFS, enemyInfo, buildViewerZombie, buildViewerBoss, buildViewerUfo,
...
function renderViewer(dt) {
  ...
  const def = ENEMY_DEFS[viewer.modelType];
  const isBossType = def && def.isBoss;
  const isUfo = viewer.modelType === 'boss_ufo';
  const r = isBossType ? (isUfo ? 24 : 18) : 10;
  const targetY = isBossType ? (isUfo ? 0.5 : 4.5) : 1.5;
