...
const skinTexture = makeCanvasTexture(192, (ctx, s) => {
  ...
  for (let i = 0; i < 70; i++) {
    ...
    if (dark) { g.addColorStop(0, `rgba(40,42,36,${0.34 + Math.random() * 0.34})`); g.addColorStop(1, 'rgba(40,42,36,0)'); }
    else { g.addColorStop(0, `rgba(190,190,190,${0.25 + Math.random() * 0.3})`); g.addColorStop(1, 'rgba(190,190,190,0)'); }
...
const faceTexture = makeCanvasTexture(192, (ctx, s) => {
  ...
  for (const ex of [0.30, 0.70]) {
    ...
    g.addColorStop(0, 'rgba(2,2,3,1)'); g.addColorStop(0.55, 'rgba(12,10,10,0.96)'); g.addColorStop(1, 'rgba(20,20,18,0)');
    ctx.fillStyle = g;
    ...
    ctx.beginPath(); ctx.ellipse(cx, cy + s * 0.075, s * 0.045, s * 0.05, 0, 0, Math.PI * 2); ctx.fill();
  }
  ...
  ctx.beginPath();
  ctx.moveTo(s * 0.26, s * 0.76); ctx.lineTo(s * 0.20, s * 0.83);
  ctx.moveTo(s * 0.74, s * 0.76); ctx.lineTo(s * 0.81, s * 0.84);
  ctx.stroke();
...
for (const sx of [-0.42, 0.42]) {
  ...
  socket.position.set(sx, 0.24, 0.78); socket.scale.set(1, 0.85, 0.6);
...
const ufoGlowMat = new THREE.MeshBasicMaterial({ color: 0x60E0FF, transparent: true, opacity: 0.85, blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide });
const ufoLightMat = new THREE.MeshBasicMaterial({ color: 0xFF2030 });
...
const ufoDisc = new THREE.Mesh(new THREE.CylinderGeometry(4.5, 5.5, 1.6, 32), ufoBodyMat);
ufoDisc.castShadow = true;
ufoRig.disc.add(ufoDisc);

const ufoRim = new THREE.Mesh(new THREE.CylinderGeometry(5.6, 5.6, 0.5, 32), ufoBodyMat);
ufoRim.position.y = -0.55;
ufoRig.disc.add(ufoRim);

const ufoDome = new THREE.Mesh(new THREE.SphereGeometry(2.4, 16, 10, 0, Math.PI * 2, 0, Math.PI / 2), ufoDomeMat);
ufoDome.position.y = 0.8;
ufoRig.disc.add(ufoDome);

const ufoCore = new THREE.Mesh(new THREE.CircleGeometry(2.0, 32), ufoGlowMat);
ufoCore.rotation.x = Math.PI / 2;
ufoCore.position.y = -0.82;
ufoRig.disc.add(ufoCore);
ufoRig.core = ufoCore;

for (let i = 0; i < 12; i++) {
  const a = (i / 12) * Math.PI * 2;
  const light = new THREE.Mesh(new THREE.SphereGeometry(0.28, 8, 6), ufoLightMat);
  light.position.set(Math.cos(a) * 5.1, -0.1, Math.sin(a) * 5.1);
  ufoRig.disc.add(light);
}

const ufoRingMat = new THREE.MeshBasicMaterial({ color: 0x60E0FF, transparent: true, opacity: 0.42, side: THREE.DoubleSide, blending: THREE.AdditiveBlending, depthWrite: false });
const ufoRing = new THREE.Mesh(new THREE.TorusGeometry(6.3, 0.20, 8, 40), ufoRingMat);
ufoRing.rotation.x = Math.PI / 2;
ufoRing.position.y = -0.20;
...
export const ENEMY_DEFS = {
  ...
  /* ---- 新增 2 种 BOSS ---- */
  boss_slam: { hp: 3200, speed: 8, dmg: 55, cloth: 0x5A0C14, skin: 0xC03040, radius: 4.2, scale: { x: 1, y: 1, z: 1 }, isBoss: true, bossKind: 'slam' },
  boss_ufo:  { hp: 2500, speed: 6, dmg: 45, cloth: 0x2A4A6A, skin: 0x8AC8E0, radius: 4.5, scale: { x: 1, y: 1, z: 1 }, isBoss: true, bossKind: 'ufo'  },
};
...
const ENEMY_I18N = {
  ...
  boss_ufo: {
    zh: { name: '🛸 幽浮母舰', desc: '悬浮在空中的金属圆盘。空中时免疫一切伤害，不断向下投掷红色光束；只有当它降落投放尸群时，才有短短的几秒暴露窗口。', stats: ['HP 2500', '空中无敌', '落地 4s 可攻'] },
    en: { name: '🛸 UFO Mothership', desc: 'A metallic disc hovering in the air. Invulnerable while airborne, raining red beams from above; only when it lands to deploy a horde does it expose a brief window.', stats: ['HP 2500', 'Immune in air', 'Attackable 4s on ground'] },
  },
};
...
export function buildViewerUfo() {
  ...
  const rim = new THREE.Mesh(new THREE.CylinderGeometry(5.6, 5.6, 0.5, 32), ufoBodyMat.clone());
  rim.position.y = -0.55; g.add(rim);
  const dome = new THREE.Mesh(new THREE.SphereGeometry(2.4, 16, 10, 0, Math.PI * 2, 0, Math.PI / 2), ufoDomeMat.clone());
  dome.position.y = 0.8; g.add(dome);
  const core = new THREE.Mesh(new THREE.CircleGeometry(2.0, 32), ufoGlowMat.clone());
  core.rotation.x = Math.PI / 2; core.position.y = -0.82; g.add(core);
  for (let i = 0; i < 12; i++) {
    const a = (i / 12) * Math.PI * 2;
    const light = new THREE.Mesh(new THREE.SphereGeometry(0.28, 8, 6), ufoLightMat.clone());
    light.position.set(Math.cos(a) * 5.1, -0.1, Math.sin(a) * 5.1); g.add(light);
  }
  const ring = new THREE.Mesh(new THREE.TorusGeometry(6.3, 0.20, 8, 40), ufoRingMat.clone());
  ring.rotation.x = Math.PI / 2; ring.position.y = -0.20; g.add(ring);
  return g;
}
