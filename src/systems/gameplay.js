...
import {
  rig, zombieMeshes, bossRig, bossAura, bossAura2, ufoRig, MAX_ZOMBIES,
...
const ENEMY_LEVEL_MULT = {
  mob: 1, bomber: 1, ranged: 1, shield: 1, elite: 0.5,
  runner: 1, dasher: 1, jumper: 1, suicide: 1,
  boss: 0.15, boss_slam: 0.15, boss_ufo: 0.15,
};
...
export function killEnemy(e) {
  ...
  if (e.type === 'suicide') {
    ...
    for (const other of queryOut) {
      ...
      if (od2 < BLAST_R * BLAST_R && od2 > 0.01) {
        const od = Math.sqrt(od2);
        other.hp -= BLAST_DMG;
        other.vel.x += odx / od * 14;
        other.vel.z += odz / od * 14;
        other.vel.y = 8;
        other.airTime = 0.9;
    ...
    if (pdx * pdx + pdz * pdz < BLAST_R * BLAST_R && player.iframe <= 0 && !player.isJumping) {
      player.hp -= BLAST_DMG;
      player.iframe = 0.6;
      sfxHurt();
    }
...
export function updateEnemies(dt) {
  ...
  for (const e of enemies) {
    ...
    if (e.jumpChargeT === undefined)   e.jumpChargeT = 0;
    if (e.slamCD === undefined)        e.slamCD = 4 + Math.random() * 2;
    if (e.slamPhase === undefined)     e.slamPhase = null;
    if (e.slamChargeT === undefined)   e.slamChargeT = 0;
    if (e.ufoPhase === undefined)      e.ufoPhase = 'air';
    if (e.ufoTimer === undefined)      e.ufoTimer = 5 + Math.random() * 3;
      ...
      if (e.pos.y <= 0) {
        ...
        if (e.type === 'jumper' && e.jumpPhase === 2) {
          ...
          for (const other of sepOut) {
            ...
            const odx = other.pos.x - e.pos.x, odz = other.pos.z - e.pos.z;
            const od2 = odx * odx + odz * odz;
            if (od2 < JR * JR && od2 > 0.01) {
              const od = Math.sqrt(od2);
              other.vel.x += odx / od * 12;
              other.vel.z += odz / od * 12;
              other.vel.y = 8;
              other.airTime = 0.8;
              other.rollSpeed = (Math.random() - 0.5) * 14;
            }
          }
          const pdx = player.pos.x - e.pos.x, pdz = player.pos.z - e.pos.z;
          if (pdx * pdx + pdz * pdz < JR * JR && player.iframe <= 0 && !player.isJumping) {
            player.hp -= JD;
            player.iframe = 0.5;
            sfxHurt();
          }
          spawnRing(e.pos.clone().setY(0.2), 0xFFA040, JR, 0.5);
          spawnBurstParticles(e.pos, 0xFFA040, 14, 12);
        ...
        /* 裂地尸王：巨型冲击波 */
          ...
          for (const other of sepOut) {
            ...
            const odx = other.pos.x - e.pos.x, odz = other.pos.z - e.pos.z;
            const od2 = odx * odx + odz * odz;
            if (od2 < BR * BR && od2 > 0.01) {
              const od = Math.sqrt(od2);
              other.vel.x += odx / od * 26;
              other.vel.z += odz / od * 26;
              other.vel.y = 14;
              other.airTime = 1.3;
              other.rollSpeed = (Math.random() - 0.5) * 22;
            }
          }
          const pdx = player.pos.x - e.pos.x, pdz = player.pos.z - e.pos.z;
          if (pdx * pdx + pdz * pdz < BR * BR && player.iframe <= 0 && !player.isJumping) {
            player.hp -= BD;
            player.iframe = 0.8;
            sfxHurt();
          }
          spawnRing(e.pos.clone().setY(0.3), 0xFF4060, BR, 0.75);
          spawnRing(e.pos.clone().setY(0.3), 0xFFD040, BR * 0.7, 0.55);
          spawnBurstParticles(e.pos, 0xFFD040, 30, 20);
          state.screenShake = 25;
          emit('screen:flash', 0.35);
          sfxThunder();
          e.slamPhase = null;
    ...
    if (e.isBoss && e.bossKind === 'slam' && e.slamPhase === 'charge') {
      ...
      if (e.slamChargeT <= 0) {
        ...
        e.airTime = 1.5;
        e.vel.y = 45;
        e.vel.x = (player.pos.x - e.pos.x) / 1.5;
        e.vel.z = (player.pos.z - e.pos.z) / 1.5;
        e.rollSpeed = (Math.random() - 0.5) * 6;
      }
      continue;
    }
    ...
    if (e.type === 'jumper' && e.jumpPhase === 1) {
      ...
      if (e.jumpChargeT <= 0) {
        ...
        e.airTime = 0.9;
        e.vel.y = 28;
        e.vel.x = (player.pos.x - e.pos.x) / 0.9;
        e.vel.z = (player.pos.z - e.pos.z) / 0.9;
        e.rollSpeed = (Math.random() - 0.5) * 8;
      }
      continue;
    }
    ...
    if (e.type === 'dasher') {
      ...
      if (e.dashCD <= 0 && distToPlayer < 22 && distToPlayer > 6) {
        ...
        const dashDist = Math.min(distToPlayer - 3, 15);
        const startPos = e.pos.clone().setY(0.3);
        e.pos.x += Math.sin(ang) * dashDist;
        e.pos.z += Math.cos(ang) * dashDist;
        spawnDodgeEffect(startPos, e.pos.clone().setY(0.3));
        sfxDodge();
        e.dashCD = 3;
      }
    }
  ...
  if (state.boss && state.boss.active) {
    ...
    if (b.bossKind === 'ufo') {
      ufoRig.root.position.set(b.pos.x, b.pos.y, b.pos.z);
      ufoRig.root.rotation.y = b.angle;
      if (ufoRig.ring) ufoRig.ring.rotation.z += dt * 1.2;
...
const UFO_FLY_TIME_MIN = 8.0;
const UFO_FLY_TIME_MAX = 12.0;
const UFO_TRANSITION_TIME = 0.9;
...
function updateUfoBoss(e, dt) {
  ...
  else if (distH < TARGET_DIST - 2) { mvX = -dirX; mvZ = -dirZ; }
  mvX += -dirZ * 0.4;
  mvZ += dirX * 0.4;
  ...
  if (e.ufoPhase === 'air') {
    ...
    if (e.ufoFireCD <= 0) {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        const cx = Math.cos(spread), sz = Math.sin(spread);
        const tx = dx * cx - dz * sz;
        const tz = dx * sz + dz * cx;
        const dy = (0.5 - e.pos.y);
        const len = Math.hypot(tx, dy, tz) || 1;
        const dirToP = new THREE.Vector3(tx / len, dy / len, tz / len);
        fxSpawnEnemyBullet(e.pos.clone().setY(e.pos.y - 1), dirToP, e.dmg * 0.6);
      }
    }
    if (e.ufoTimer <= 0) {
      e.ufoPhase = 'landing';
      e.ufoTimer = UFO_TRANSITION_TIME;
    }
  } else if (e.ufoPhase === 'landing') {
    const t = 1 - Math.max(0, e.ufoTimer / UFO_TRANSITION_TIME);
    e.pos.y = UFO_AIR_HEIGHT * (1 - t);
    e.invulnerable = true;
    if (e.ufoTimer <= 0) {
      e.ufoPhase = 'ground';
      e.ufoTimer = UFO_GROUND_TIME;
      e.pos.y = 0;
      ...
      for (let i = 0; i < 8; i++) {
        ...
        spawnEnemy('mob', e.pos.x + Math.cos(a) * r, e.pos.z + Math.sin(a) * r);
      }
      sfxBossSpawn();
    }
  } else if (e.ufoPhase === 'ground') {
    e.pos.y = 0;
  ...
  } else if (e.ufoPhase === 'rising') {
    const t = 1 - Math.max(0, e.ufoTimer / UFO_TRANSITION_TIME);
    e.pos.y = UFO_AIR_HEIGHT * t;
    e.invulnerable = true;
    if (e.ufoTimer <= 0) {
      e.ufoPhase = 'air';
      e.ufoTimer = UFO_FLY_TIME_MIN + Math.random() * (UFO_FLY_TIME_MAX - UFO_FLY_TIME_MIN);
      e.ufoFireCD = 1.0;
    }
  }
}
  ...
  for (const e of queryOut) {
    ...
    if (dx * dx + dz * dz < C.hitRadius * C.hitRadius) {
      ...
      if (skills.shock.active) {
        ...
        for (const other of queryOut) {
          if (other === e || !other.active || other.isBoss || other.invulnerable) continue;
          const odx = other.pos.x - e.pos.x, odz = other.pos.z - e.pos.z;
